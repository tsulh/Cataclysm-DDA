...
monster::monster( const mtype_id &id ) : monster()
{
    ...
    reproduces = type->reproduces && type->baby_timer && !monster::has_flag( MF_NO_BREED ) && !neutered;
...
bool monster::is_reproducing()
{
    return reproduces;
}

bool monster::is_neutered()
{
    return neutered;
}

void monster::make_neutered()
{
    neutered = true;
}
